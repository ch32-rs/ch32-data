...
impl ChipInterrupts {
    pub fn parse() -> anyhow::Result<Self> {
        ...
        for f in files {
            ...
            let strings: Vec<_> = parsed
                ...
                .filter(|irq| {
                    !parsed.version.starts_with("STM32F3") || !irq.comment.contains("remap")
                })
                .map(|irq| irq.value)
                .collect();
    ...
    pub(crate) fn process(
        ...
        trace!(
            "parsing interrupts for chip {} core {}",
            chip_name,
            core.name
        );
        ...
        core.nvic_priority_bits = header_defines
            .0
            .get("__NVIC_PRIO_BITS")
            .map(|bits| *bits as u8);
        ...
        core.interrupts = header_irqs
            .iter()
            .map(|(k, v)| ch32_data_serde::chip::core::Interrupt {
                name: k.clone(),
                number: *v,
            })
            .collect();
        ...
        let chip_nvic = group
            ...
            .ok_or_else(|| {
                format!("couldn't find nvic. chip_name={chip_name} want_nvic_name={want_nvic_name}")
            })
            .unwrap();
                ...
                for (p, mut ss) in peri_signals.into_iter() {
                    ...
                    for s in ss {
                        if !known.contains(&s.clone()) {
                            panic!(
                                "Unknown signal {s} for peri {p}, known={known:?}, parts={parts:?}"
                            );
                        }
                        trace!("    signal: {} {}", p, s);
        ...
        for p in &mut core.peripherals {
            if let Some(signals) = chip_signals.get(&p.name) {
                let mut all_irqs: Vec<stm32_data_serde::chip::core::peripheral::Interrupt> =
                    Vec::new();
...
fn tokenize_name(name: &str) -> Vec<String> {
    ...
    let r = regex!(
        r"(SPDIF_RX|EP\d+_(IN|OUT)|OTG_FS|OTG_HS|USB_DRD_FS|USB_FS|C1_RX|C1_TX|C2_RX|C2_TX|[A-Z0-9]+(_\d+)*)_*"
    );
